	...
	buildCode : function( url, response ){
		...
		var name		= decodeURIComponent( response.name ),
			shortcode	= '[rnkrwp ' + 'id="' + response.id + '" ' + 
				'format="' + ( ( typeof response.listFormatType === 'undefined' || response.listFormatType === '' ) ? 'grid' :
					( response.listFormatType === 'BLOG' ? 'slideshow' : response.listFormatType.toLowerCase() ) ) + '"]';
